...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        if goal_index == len(waypoints) - 1:
            delta_x = np.array(waypoints[goal_index][0]) - np.array(waypoints[goal_index - 1][0])
            delta_y = np.array(waypoints[goal_index][1]) - np.array(waypoints[goal_index - 1][1])
        else:
            delta_x = np.array(waypoints[goal_index + 1][0]) - np.array(waypoints[goal_index][0])
            delta_y = np.array(waypoints[goal_index + 1][1]) - np.array(waypoints[goal_index][1])
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        transformation_matrix = np.array(((np.cos(-ego_state[2]), -np.sin(-ego_state[2])), (np.sin(-ego_state[2]), np.cos(-ego_state[2]))))
        goal_state_transformed = transformation_matrix @ np.array(goal_state_local[:2]).T

        goal_x = goal_state_transformed[0]
        goal_y = goal_state_transformed[1]
        # ------------------------------------------------------------------
